...
if (isset($_GET['idlh'])) {
	$idlh = $_GET['idlh'];
	$sql2 = "SELECT * FROM HOA WHERE MA_LOAIHOA = '".$idlh."'";
	$query2= mysqli_query($con,$sql2);
									//lấy tên chủ đề
	$sql_getname = "SELECT * FROM LOAIHOA WHERE MA_LOAIHOA = '".$idlh."'";
	$query_getname = mysqli_query($con,$sql_getname);
	$row_getname = mysqli_fetch_assoc($query_getname);

}else{
	$sql2 = "SELECT * FROM HOA";
	$query2= mysqli_query($con,$sql2);
	$flat = 0;
}
?>
<!-- Sản phẩm -->
<div class="col-md-9">
	<h3 class="title-hoa"><?php if($flat == 0){ echo "Tất cả Hoa";}else{ echo $row_getname['TEN_LOAIHOA'];}?></h3>
	<div class="row">
		<?php 
